...
const TERM_GLOSSARY = {
  ...
  workflow: {
    ...
    body: "先寫人怎麼做，再標哪些步給 AI、哪些留給人，然後用 <strong>SPEC／Claude Skill</strong> 把約定寫清楚，再用 Hook、MCP、Subagent 把它變成可重複執行的路徑。",
  },
  spec: {
    title: "SPEC（規格）",
  ...
  skill: {
    title: "Claude Skill（技能／工作手冊）",
    metaphor: "比喻：把「這次怎麼做出來的」寫成單位共用 SOP，下次直接照做",
    body: "Claude Skill 不是再接一條 MCP，而是把<strong>怎麼做這類任務</strong>寫成可重用說明（步驟、規則、該看哪些檔、輸出格式）。之後對話可自動套用，減少每次重講 PROMPT。<br /><br /><strong>建議怎麼做：</strong><br />1. 先用黃金案例跑通一次，記下成功步驟與踩坑。<br />2. 濃縮成短手冊：何時用、必做／禁止、輸出格式長怎樣。<br />3. 放進專案（例如 Claude Skill／規則），讓同事同一條路徑可重複。<br />4. 真正接工具、門禁、分工，仍搭配 Hook／MCP／Subagent——Skill 管「怎麼做」，它們管「怎麼接、怎麼攔、怎麼拆」。",
  },
  "collab-tools": {
...
const TOOL_COPY = {
  ...
  workflow: {
    ...
    items: ["先寫 5～9 步現況流程", "標哪些步 AI 做、哪些人做", "知識庫用 Markdown；約定寫成 SPEC／Claude Skill", "門禁用 Hook；外部系統用 MCP；分工用 Subagent"],
  },
};
...
const WF_COPY = {
  ...
  3: { title: "3 · 寫成 SPEC／Claude Skill", body: "輸入、輸出、例外、驗收寫清楚。再濃縮成專案 Claude Skill（可重用工作手冊），讓之後對話自動套用，同事不用每次重講。" },
