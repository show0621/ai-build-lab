 ...
 * Demo is modeled after TP AI NOTE (三欄專業知識庫工作台) so prompts are practical. */

const DEMO_BLOCKS = [
  {
    title: "區塊0｜整體版面與視覺（對齊 TP AI NOTE）",
    body: `【參考產品】TP AI NOTE（移轉訂價智慧知識庫）的工作台外觀與密度。
請做出「同類型」專業全螢幕工作台，不是行銷落地頁、不是空白後台。
...
- 問答區一進來要有歡迎訊息，說明怎麼問、會依知識庫回答`,
  },
  {
    ...
    body: `對齊 TP AI NOTE：左「知識拓撲」+ 右上「節點解析」+ 右下「內部問答」。
...
一主題一檔或一條一塊；front matter：title/version/owner/tags/生效日；易混淆主題分檔`,
  },
  {
    ...
    body: `可做成右下模式之一，或頂部主 Tab；外觀要與 TP AI NOTE 同一套設計系統。
...
■ 外觀細節
- 延續玻璃面板＋圓角；翻譯輔助用琥珀／黃色點綴（類似 TP NOTE 譯文黃窗）
- 對照區要好讀：等寬或清楚段落間距，不要擠成一團`,
  },
  {
    title: "區塊3｜一鍵去識別化（即時預覽＋對照表）",
    body: `對齊 TP AI NOTE 聊天區「去識別化」能力，但做成完整可視工作區。
...
- 與整體工作台同一設計語言（白面板、圓角、輕陰影）`,
  ...
  {
    title: "區塊4｜筆記本與紀錄",
    body: `對齊 TP AI NOTE「存筆記本／匯出筆記」後的管理需求。
...
■ 外觀
- 列表選中態清楚（左邊色條或淡底）
- 筆記閱讀可用稍大行高；可選文青／襯線閱讀樣式（參考 TP NOTE 筆記風格），但介面控制項仍用無襯線
- 空狀態：「尚無筆記，先去問答區存一則」`,
  },
  {
    title: "區塊5｜上傳入庫與檔案管理（次要）",
    body: `對齊 TP AI NOTE 頂欄「上傳知識庫 PDF」流程。
...
### A. 第一優先：可打開、好看、可點的工作台 UI（對齊 TP AI NOTE）
...
## 0. 文件說明
- 參考實作標竿：**TP AI NOTE（移轉訂價智慧知識庫）** 的版面、密度與操作動線
- 目標：做出「同類專業工作台」——不是行銷頁、不是只有資料夾的專案骨架
...
- 上傳入庫有模態流程；參數可配置
- UI 達到「像 TP AI NOTE 一樣能給內行人用」的完成度
...
請做出一個「像 TP AI NOTE（移轉訂價智慧知識庫）那樣」的專業單頁工作台，不是行銷頁，也不是只有資料夾結構。
...
## 開始前
用 ≤5 點複述需求，必須包含：「會先做出像 TP AI NOTE 的三區工作台 UI，不會只交結構檔」。
然後立刻開始做第 1 回合畫面。`;
}
